...
void ProcessPacket(int ci, unsigned char packet[])
{
	...
	case SC_POS: {
		...
		if (pos_packet->id < NPC_ID_START) {
			int my_id = client_map[pos_packet->id];
			if (0 != client_map[my_id]) {
				g_clients[my_id].x = pos_packet->x;
				g_clients[my_id].y = pos_packet->y;
			}
			if (ci == my_id) {
				if (pos_packet->seq_no > 10) {
					auto delay = high_resolution_clock::now() - g_clients[my_id].last_move_time;
					auto d_ms = static_cast<int>(duration_cast<milliseconds>(delay).count());
					if (pos_packet->seq_no != g_clients[my_id].seq_no) d_ms = 1000;
					if (0 == global_delay) global_delay = d_ms;
					else if (global_delay < d_ms) global_delay++;
					else if (global_delay > d_ms) global_delay--;

					if (global_delay >= 1000) g_stop_add_load = true;
				}
			}
		}
...
void Adjust_Number_Of_Client()
{
	...
	// if (g_stop_add_load) return;
	int delay = 50;
	int t_delay = global_delay;
	if (t_delay >= 1000) return;
	if (0 != t_delay) {
		delay = rand() % t_delay;
		delay = max(50, delay);
	}
	auto duration = high_resolution_clock::now() - last_connect_time;
	if (delay > duration_cast<milliseconds>(duration).count()) return;
...
void Test_Thread()
{
	while (true) {
		...
		for (int i = 0; i < num_connections; ++i) {
			...
			my_packet.seq_no = ++g_clients[i].seq_no;
