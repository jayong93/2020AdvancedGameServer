...
//	auto list_head = t_epoch_list.load();
//	while (true) {
//		my_epoch.next.store(list_head);
...
//	const T& peek() const;
//	T& peek();
//	uint64_t size() const { return num_node.load(); }
//};
//
//template<typename T>
...
//	my_epoch.epoch.store(g_epoch.load());
//}
//
//template<typename T>
//inline void MPSCQueue<T>::end_op() const
//{
//	my_epoch.epoch.store(ULLONG_MAX);
//}
//
//template<typename T>
//inline void MPSCQueue<T>::retire(QueueNode<T>* node)
//{
//	RetiredList::get().emplace_back(node, g_epoch.load());
//	counter++;
//	if (counter % epoch_increase_rate == 0) { g_epoch.fetch_add(1); }
...
//	auto thread_epoch = t_epoch_list.load();
//	while (true) {
//		auto epoch = thread_epoch->epoch.load();
//		if (min_epoch > epoch) { min_epoch = epoch; }
//
//		auto next_t_epoch = thread_epoch->next.load();
...
//	QueueNode<T>* next_head = head->next.load();
//
