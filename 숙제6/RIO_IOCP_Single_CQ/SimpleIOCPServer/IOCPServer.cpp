...
void do_worker(int t_id)
{
	...
	while (true) {
		...
		auto num_result = rio_ftable.RIODequeueCompletion(rio_cq, results, 100);
		rio_ftable.RIONotify(rio_cq); // client의 rio_cq에서 필요한 만큼 dequeue를 끝냈으므로 다른 worker thread에게 처리를 넘겨도 됨.
